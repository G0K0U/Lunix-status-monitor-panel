...
enum kinds {
	...
	aquastreamult, poweradjust3, highflow, mpspressure
};
...
static bool mps_pressure;
module_param(mps_pressure, bool, 0444);
MODULE_PARM_DESC(mps_pressure,
		 "Interpret USB product 0xf003 as an MPS Pressure device");
...
/* Raw fields shared by MPS Pressure devices using product ID 0xf003. */
#define MPSPRESSURE_NUM_READINGS	4
#define MPSPRESSURE_RAW_OFFSET		0x11
#define MPSPRESSURE_ZERO_OFFSET		0x19
#define MPSPRESSURE_NORMALIZED_OFFSET	0x1b
#define MPSPRESSURE_RESULT_OFFSET	0x23
...
static const char *const label_mpspressure_speeds[] = {
	"Pressure ADC [raw]",
	"Pressure zero offset [raw]",
	"Pressure normalized [raw]",
	"Pressure result [raw]"
};
...
static int aqc_legacy_read(struct aqc_data *priv)
{
	...
	case highflow:
		...
		break;
	case mpspressure:
		/* Keep pressure fields raw until their proprietary scaling is known. */
		priv->speed_input[0] = get_unaligned_le16(priv->buffer +
							  MPSPRESSURE_RAW_OFFSET);
		priv->speed_input[1] = (s16)get_unaligned_le16(priv->buffer +
							       MPSPRESSURE_ZERO_OFFSET);
		priv->speed_input[2] = get_unaligned_le16(priv->buffer +
							  MPSPRESSURE_NORMALIZED_OFFSET);
		priv->speed_input[3] = get_unaligned_le16(priv->buffer +
							  MPSPRESSURE_RESULT_OFFSET);
		break;
	case poweradjust3:
...
static int aqc_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	case USB_PRODUCT_ID_HIGHFLOW:
		priv->kind = mps_pressure ? mpspressure : highflow;

		priv->num_fans = 0;

		priv->num_temp_sensors = HIGHFLOW_NUM_SENSORS;
		priv->temp_sensor_start_offset = HIGHFLOW_SENSOR_START;
		priv->num_flow_sensors = mps_pressure ? MPSPRESSURE_NUM_READINGS :
							      HIGHFLOW_NUM_FLOW_SENSORS;
		...
		priv->speed_label = mps_pressure ? label_mpspressure_speeds :
							 label_highflow_speeds;
		break;
	default:
		break;
	}
